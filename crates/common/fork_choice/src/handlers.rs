...
pub async fn on_block(
    ...
    let is_before_attesting_interval = time_into_slot < network_spec().seconds_per_slot / INTERVALS_PER_SLOT;
...
pub fn on_tick(store: &mut Store, time: u64) -> anyhow::Result<()> {
    ...
    let tick_slot = (time - store.db.genesis_time_provider().get()?) / network_spec().seconds_per_slot;
