...
impl Store {
    ...
    /// Returns true if the block with `root` was imported optimistically and has not yet been fully
    /// validated.
    pub fn is_optimistic(&self, root: B256) -> anyhow::Result<bool> {
        Ok(self
            .db
    ...
    pub fn is_head_optimistic(&self) -> anyhow::Result<bool> {
        let head = self.get_head()?;
        self.is_optimistic(head)
    }
