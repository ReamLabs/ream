...
pub fn compute_sync_committee_period(epoch: u64) -> anyhow::Result<u64> {
    Ok(epoch / EPOCHS_PER_SYNC_COMMITTEE_PERIOD)
}
...
pub fn compute_subnets_for_sync_committee(
    ...
    let subnet_ids: HashSet<u8> = sync_committee_indices
        .iter()
        .map(|&index| ((index as u64) / (SYNC_COMMITTEE_SIZE / SYNC_COMMITTEE_SUBNET_COUNT)) as u8)
        .collect();
