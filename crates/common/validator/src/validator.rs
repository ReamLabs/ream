...
use ream_consensus::{constants::SLOTS_PER_EPOCH, electra::beacon_state::BeaconState, misc::compute_start_slot_at_epoch};

pub fn check_if_validator_active(state: &BeaconState, validator_index: u64) -> bool {
    let validator = &state.validators[validator_index as usize];
    validator.is_active_validator(state.get_current_epoch())
}
...
pub fn get_committee_assignment(state: &BeaconState, epoch: u64, validator_index: u64) -> Result<Option<(Vec<u64>, u64, u64)>> {
    let next_epoch = state.get_current_epoch() + 1;
    ensure!(epoch <= next_epoch, "Epoch can not be greater than next epoch.");
