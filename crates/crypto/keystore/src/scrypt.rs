...
fn scrypt_romix(block_size: usize, input_output: &mut [u8], cost_parameter: usize) {
    ...
    for _ in 0..cost_parameter {
        let memory_index = {
            let last_eight_bytes =
                &input_output[block_size_in_bytes - 64..block_size_in_bytes - 56];
            u64::from_le_bytes(last_eight_bytes.try_into().unwrap()) as usize & (cost_parameter - 1)
        };
