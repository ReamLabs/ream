            ...
            tokio::select! {
                ...
                Some(event) = p2p_receiver.recv() => {
                    match event {
                        ...
                        P2PMessage::Gossip { topic, data } => {
                            let ident_topic: libp2p::gossipsub::IdentTopic = topic.into();
                            if let Err(err) = self.swarm.behaviour_mut().gossipsub.publish(ident_topic, data) {
