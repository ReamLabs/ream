            ...
            mod tests_process_execution_payload {
                ...
                async fn test_operation() {
                    ...
                    for entry in std::fs::read_dir(base_path).unwrap() {
                        ...
                        let mock_engine = MockExecutionEngine::new(&case_dir.join("execution.yaml")).expect("romove result");
                        let result = state.process_execution_payload(&input, &mock_engine).await;
