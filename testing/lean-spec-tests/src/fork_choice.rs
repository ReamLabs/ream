...
use ream_network_spec::networks::{
    Devnet, LEAN_NETWORK_SPEC, LeanNetworkSpec, lean_network_spec, set_lean_network_spec,
};
...
pub async fn run_fork_choice_test(test_name: &str, test: ForkChoiceTest) -> anyhow::Result<()> {
    ...
    if LEAN_NETWORK_SPEC.get().is_none() {
        let genesis_time = test.anchor_state.config.genesis_time;
        let num_validators = test.anchor_state.validators.data.len() as u64;
    ...
    let block = Block::from(&test.anchor_block);
    ...
    // Setup test database
    let test_dir = setup_data_dir("lean_spec_tests", None, true)
    ...
    for (index, step) in test.steps.iter().enumerate() {
        match step {
            ...
            ForkChoiceStep::Block {
                ...
                let ream_block = Block::from(block);
                ...
                // Create 1 blank signature for the proposer attestation
                let signatures = VariableList::from(vec![Signature::blank()]);

                let result = store
                    .on_block(&SignedBlockWithAttestation {
                        message: BlockWithAttestation {
                            proposer_attestation: Attestation {
